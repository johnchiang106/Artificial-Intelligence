...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        # in the order [below, left, above, right]
        moves = [(1, 0), (0, -1), (-1, 0), (0, 1)]

        for move in moves:
            new_zero_row = zero_row + move[0]
            new_zero_col = zero_col + move[1]

            if 0 <= new_zero_row < len(self.state) and 0 <= new_zero_col < len(self.state[0]):
                new_state = [list(row) for row in self.state]
                new_state[zero_row][zero_col], new_state[new_zero_row][new_zero_col] = (
                    new_state[new_zero_row][new_zero_col],
                    new_state[zero_row][zero_col],
                )
                neighbor = EightPuzzleState(
                    new_state,
                    self.goal,
                    dist_from_start=self.dist_from_start + 1,
                    use_heuristic=self.use_heuristic,
                    zero_loc=(new_zero_row, new_zero_col),
                )
    ...
    def compute_heuristic(self):
        ...
        for i in range(len(self.state)):
            for j in range(len(self.state[i])):
                if self.state[i][j] != 0:
                    # Calculate the Manhattan distance for each tile (excluding the empty tile)
                    goal_row, goal_col = divmod(self.state[i][j], len(self.state))
