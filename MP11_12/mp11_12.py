    ...
    def execute(self):
        if self.args.early_check:
            ...
            if not np.array_equal(test_results, gold_results):
                ...
                #     file.write(str(gold_results) + "\n")
                # diff_positions = np.where(test_results != gold_results)
                # for pos in zip(*diff_positions):
                #     print(f"Index: {pos}, Value in test_results: {test_results[pos]}, Value in gold_results: {gold_results[pos]}")
                match = False
            else:
