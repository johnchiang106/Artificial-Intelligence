...
class SingleGoalGridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        # Create SingleGoalGridState objects for each neighboring location
        for neighbor_loc in neighboring_grid_locs:
            neighbor_state = SingleGoalGridState(neighbor_loc, self.goal, 
...
class GridState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        # Create GridState objects for each neighboring location
        for neighbor_loc in neighboring_locs:
            if neighbor_loc in self.goal:
                newGoal = tuple(item for item in self.goal if item != neighbor_loc)
            else:
                newGoal = self.goal
                
            neighbor_state = GridState(neighbor_loc, newGoal, 
                                       dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, 
                                       maze_neighbors=self.maze_neighbors, mst_cache=self.mst_cache)
            nbr_states.append(neighbor_state)
    ...
    def __eq__(self, other):
        return self.state == other.state and set(self.goal).issubset(other.goal)
    ...
    def compute_heuristic(self):
        # If there's only one goal, MST value is zero, so the heuristic is just manhattan distance to the nearest goal
        if len(self.goal) == 0:
            return 0
        ...
        # Compute the MST value for the set of goals
        if self.goal in self.mst_cache:
            mst_value = self.mst_cache[self.goal]
        else:
            mst_value = compute_mst_cost(self.goal, manhattan)
            self.mst_cache[self.goal] = mst_value

        closest_distance = min(manhattan(self.state, g) for g in self.goal)

        # Calculate the heuristic as described
        return closest_distance + mst_value
    
    # TODO(VI): implement this method... should be unchanged from before
    def __lt__(self, other):
        # Compare states based on f = g + h, and use tiebreak_idx to break ties
        f_self = self.dist_from_start + self.h
        f_other = other.dist_from_start + other.h
        if f_self == f_other:
            return self.tiebreak_idx < other.tiebreak_idx
