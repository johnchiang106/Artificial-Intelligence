...
class Maze:
    def __init__(self, alien, walls, waypoints, goals, move_cache={}, k=5, use_heuristic=True):
        ...
        self.__waypoints = waypoints + goals #+ [alien.get_centroid()]
    ...
    def filter_valid_waypoints(self):
        ...
        width = self.alien.get_alien_limits()[0][1]
        height = self.alien.get_alien_limits()[1][1]
        for i in range( len(self.alien.get_shapes()) ):
            for waypoint in self.__waypoints:
                x, y = waypoint
                new_alien = self.create_new_alien(x, y, i)
                if (not does_alien_touch_wall(new_alien, self.walls)) and (is_alien_within_window(new_alien, (width,height))):
                # if not does_alien_touch_wall(new_alien, self.walls):
                    valid_waypoints[i].append((x, y))
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        ...
        """
        nearest_neighbors = []
        waypoints = self.__valid_waypoints[cur_shape]
        waypoints.sort(key=lambda waypoint: euclidean_distance((*cur_waypoint,cur_shape), (*waypoint,cur_shape)))
        if waypoints[0] == cur_waypoint:
            del waypoints[0]
        for waypoint in waypoints:
            if self.is_valid_move((cur_waypoint[0], cur_waypoint[1], cur_shape), (waypoint[0], waypoint[1], cur_shape)):
                nearest_neighbors.append(waypoint)
                if len(nearest_neighbors) == self.k:
                    break
        # nearest_neighbors.extend(waypoints[:self.k])
    ...
    def is_valid_move(self, start, end):
        ...
        """
        if not (0 <= end[2] < len(self.alien.get_shapes())):
            return False
        
        # The start point of maze is not in waypoints, need to be checked
        if end[:2] == self.get_start().get_state()[:2]:
            width = self.alien.get_alien_limits()[0][1]
            height = self.alien.get_alien_limits()[1][1]
        ...
        # if start[:2] == end[:2]:
        #     return tuple(end[:2]) in self.__valid_waypoints[end[2]]
        if tuple(end[:2]) not in self.__valid_waypoints[end[2]]:
            return False
        if start[:2] == end[:2]: # Only changing shape
            return True
        
        start_alien = self.create_new_alien(*start)
        if does_alien_path_touch_wall(start_alien, self.walls, tuple(end[:2])):
            return False
