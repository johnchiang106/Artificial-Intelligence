...
def training(sentences):
    ...
    for tag in init_prob:
        init_prob[tag] = (init_prob[tag] + epsilon_for_pt) / (len(sentences) + epsilon_for_pt * (len(init_prob) + 1))
    ...
    for tag in all_tags:
        ...
        emit_prob[tag]["UNKNOWN"] = (epsilon_for_pt) / (tag_count[tag] + epsilon_for_pt * (len(emit_prob[tag]) + 1))
        
        for next_tag in all_tags:
            trans_prob[tag][next_tag] = (trans_prob[tag][next_tag] + epsilon_for_pt) / (tag_count[tag] + epsilon_for_pt * (len(all_tags) + 1))
...
def viterbi_stepforward(i, word, prev_prob, prev_predict_tag_seq, emit_prob, trans_prob):
    ...
    :param prev_prob: A dictionary of tags to probs representing the max probability of getting to each tag at in the previous column of the lattice
    :param prev_predict_tag_seq: A dictionary representing the predicted tag sequences leading up to the previous column of the lattice for each tag in the previous column
    ...
    if i == 0:
        for tag in emit_prob:
            log_prob[tag] = prev_prob[tag] + math.log(emit_prob[tag].get(word, emit_prob[tag]["UNKNOWN"]))
        ...
        for tag in emit_prob:
            ...
            for prev_tag in emit_prob:
                prob = prev_prob[prev_tag] + math.log(trans_prob[prev_tag][tag]) + math.log(emit_prob[tag].get(word, emit_prob[tag]["UNKNOWN"]))
                if prob > max_prob:
                    max_prob = prob
